...
function ExplorarPostulantes() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await getEmployeesFiltered({field: "municipio", operator: "==", target: "Monterrey"});
      setEmployees(result);
    };
    fetchData();
  }, []);
  ...
  //const employees = await getEmployeesFiltered();
        ...
        <Row className="mx-auto">
          <Col md={{size: 10, offset: 1}} sm={{size: 12}}>
            ...
            <Table>
              ...
              <tbody>
              { employees.users.map((postulante:any, index:number) => {
